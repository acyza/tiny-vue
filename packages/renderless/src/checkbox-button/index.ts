...
export const computedGetModelSet = ({ state, props, emit, dispatch, constants }) => (value) => {
  if (state.checkboxGroup) {
...
export const computedCheckboxGroup = ({ parent, constants }) => () => {
...
export const computedSize = ({ state, formItemSize }) => () => (state.checkboxGroup && state.checkboxGroup.state.checkboxGroupSize) || formItemSize.value
