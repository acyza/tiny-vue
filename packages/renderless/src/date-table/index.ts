...
export const getWeeks = ({ props, state }) => () => {
  const week = props.firstDayOfWeek
...
export const getCell = ({ state, props }) => (row, i, j) => {
...
export const getRows = ({ api, props, state, t, vm }) => () => {
...
export const watchMinDate = ({ api, props }) => (value, oldvalue) => {
  if (getDateTimestamp(value) !== getDateTimestamp(oldvalue)) {
    api.markRange(props.minDate, props.maxDate)
  }
}

export const watchMaxDate = ({ api, props }) => (value, oldvalue) => {
  if (getDateTimestamp(value) !== getDateTimestamp(oldvalue)) {
    api.markRange(props.minDate, props.maxDate)
  }
}
...
export const getCellRangeClasses = ({ props }) => (cell, classes) => {
...
export const getCellClasses = ({ api, props, state }) => (cell) => {
  let classes = []
...
export const getDateOfCell = ({ props, state }) => (row, column) => {
...
export const isWeekActive = ({ props, state }) => (cell) => {
  if (props.selectionMode !== DATEPICKER.Week) {
    return false
  }
...
export const handleClick = ({ api, emit, props, state }) => (event) => {
