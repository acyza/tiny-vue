...
export const getConfig = ({ parent, props }) => () => {
...
export const computClearVisible = ({ props, state }) => () => {
...
export const computePresentContent = ({ api, state }) => () => {
...
export const watchCheckedValue = ({ api, emit, state }) => (value) => {
...
export const selfMounted = ({ api, parent, props, refs, state }) => () => {
  const { input } = refs
...
export const toggleDropDownVisible = ({ emit, nextTick, refs, state, updatePopper }) => (visible) => {
  if (state.isDisabled) {
    return
  }
...
export const handleBlur = ({ constants, dispatch, emit, state }) => (e) => {
...
export const handleInput = ({ api, state, refs }) => (val, event) => {
...
export const handleExpandChange = ({ constants, dispatch, emit, nextTick, state, updatePopper }) => (value) => {
  nextTick(() => {
    dispatch(constants.COMPONENT_NAME.FormItem, constants.EVENT_NAME.FormBlur, [state.multiple ? state.presentText : state.inputValue])
...
export const focusFirstNode = ({ refs, state }) => () => {
...
export const computePresentText = ({ props, state }) => () => {
...
export const computePresentTags = ({ api, props, state }) => () => {
...
export const getSuggestions = ({ nextTick, props, state, updatePopper }) => () => {
...
export const handleSuggestionKeyDown = ({ api }) => (event) => {
  event = event || window.event
...
export const handleDelete = ({ api, state }) => () => {
...
export const handleSuggestionClick = ({ api, state }) => (index) => {
...
export const deleteTag = ({ emit, state }) => (index) => {
...
export const updateStyle = ({ parent, refs, state, updatePopper }) => () => {
  const $el = parent.$el
...
export const setInputHeightToTag = ({ nextTick, parent }) => () => {
  nextTick(() => {
...
export const handleBeforeUpdate = ({ props, api }) => () => {
  if (!isEmpty(props.modelValue)) {
    api.computePresentContent()
  }
}
